    ...
    */
    ...
    function queryPlatform(qid, businessParams) {
        return new Promise(function (resolve, reject) {
            if (!qid) { reject(new Error('文件详情查询号尚未配置')); return; }
            if (!global.isqrydata || typeof global.isqrydata.query !== 'function') { reject(new Error('isqrydata.js 未加载')); return; }
            var params = commonParams(); Object.keys(businessParams || {}).forEach(function (key) { params[key] = businessParams[key]; });
    ...
    function enrichDocument(item) {
        ...
        documentItem.lastParseTime = documentItem.lastParseTime || documentItem.collectTime;
        documentItem.parserName = documentItem.parserClass || profile.parserClass || '待识别';
        if (documentItem.parseStatus === '解析失败') { documentItem.parseMessage = '处理接口未能识别文件中的样品编号列，请进入解析工作页重新解析。'; }
        return documentItem;
    }
    function mockLoadDetail(fdiseq) {
        ...
        var item = documents.find(function (record) { return record.fdiseq === fdiseq; });
        if (!item && (!fdiseq || fdiseq === fallbackDocument.fdiseq)) { item = fallbackDocument; }
    ...
    function statusClass(status) { return status === '解析成功' ? 'success' : 'error'; }
    ...
    function renderInfo(item) {
        setText('headerFileName', item.fileName); setText('headerFileId', '文件标识：' + item.fdiseq); setText('fileName', item.fileName); setText('fileType', item.fileType);
        setText('fileSize', formatFileSize(item.fileSize)); setText('fileId', item.fdiseq); setText('deviceName', item.deviceName);
        setText('instno', item.instno); setText('collectTime', item.collectTime); setText('collectMode', item.collectMode);
        setText('parseStatus', item.parseStatus); el('parseStatus').className = 'status-text ' + statusClass(item.parseStatus);
        setText('dataCount', Number(item.dataCount || 0).toLocaleString('zh-CN') + ' 条'); setText('lastParseTime', item.lastParseTime); setText('parserName', item.parserName);
        el('parseMessageRow').classList.toggle('is-hidden', !item.parseMessage); setText('parseMessage', item.parseMessage);
        el('previewType').textContent = item.fileType + ' 文件';
    }
    function profileFor(item) {
        return interfaceProfiles[item.instno] || { parserClass: item.parserClass || '待识别', columns: ['样品编号', '检测项目', '结果'], rows: [] };
    }
    function renderTabularPreview(profile) {
        var table = document.querySelector('.preview-table');
        table.style.minWidth = Math.max(900, (profile.columns.length + 1) * 140) + 'px';
        el('excelHead').innerHTML = '<tr><th>序号</th>' + profile.columns.map(function (column) { return '<th title="' + escapeHtml(column) + '">' + escapeHtml(column) + '</th>'; }).join('') + '</tr>';
        el('excelRows').innerHTML = profile.rows.map(function (row, index) {
            return '<tr><td>' + (index + 1) + '</td>' + row.map(function (value) { return '<td title="' + escapeHtml(value) + '">' + escapeHtml(value) + '</td>'; }).join('') + '</tr>';
        }).join('');
    ...
    function renderPdfPage() {
        ...
        var pages = [
            '<h3>' + escapeHtml(profile.reportTitle || '仪器检测分析报告') + '</h3><p>文件序号：' + escapeHtml(currentDocument.fdiseq) + '</p><p>仪器编号：' + escapeHtml(currentDocument.instno) + '</p><p>仪器设备：' + escapeHtml(currentDocument.deviceName) + '</p><p>检测日期：' + escapeHtml(currentDocument.collectTime.slice(0,10)) + '</p><table><tr><td>解析程序</td><td>' + escapeHtml(currentDocument.parserName) + '</td><td>解析数据量</td><td>' + escapeHtml(currentDocument.dataCount) + ' 条</td></tr></table>',
            '<h3>检测结果</h3>' + resultTable
        ];
        el('pdfPaper').innerHTML = pages[pdfPage - 1]; el('pdfPage').textContent = pdfPage; el('pdfTotal').textContent = pages.length;
        el('pdfPrevious').disabled = pdfPage <= 1; el('pdfNext').disabled = pdfPage >= pages.length;
    }
    function renderPreview(item) {
        var profile = profileFor(item);
        ['excelPreview', 'csvPreview', 'pdfPreview', 'imagePreview', 'unsupportedPreview'].forEach(function (id) { el(id).classList.add('is-hidden'); });
        if (item.fileType === 'Excel') { renderTabularPreview(profile); el('excelPreview').classList.remove('is-hidden'); return; }
        if (item.fileType === 'CSV' || item.fileType === 'TXT') {
            el('textPreviewTitle').textContent = item.fileType + ' 数据预览';
            el('textPreviewEncoding').textContent = profile.encoding || (item.fileType === 'CSV' ? 'UTF-8 · 逗号分隔' : 'UTF-8');
            el('csvText').textContent = profile.columns.join('\t') + '\n' + profile.rows.map(function (row) { return row.join('\t'); }).join('\n');
            el('csvPreview').classList.remove('is-hidden'); return;
        }
        if (item.fileType === 'PDF') { pdfPage = 1; renderPdfPage(); el('pdfPreview').classList.remove('is-hidden'); return; }
        if (item.fileType === '图像') { imageScale = 1; updateImageScale(); el('imagePreview').classList.remove('is-hidden'); return; }
        el('unsupportedPreview').classList.remove('is-hidden');
    }
    function updateImageScale() { el('spectrumImage').style.transform = 'scale(' + imageScale + ')'; el('imageScale').textContent = Math.round(imageScale * 100) + '%'; }
    function showToast(message) { var toast = el('toast'); toast.textContent = message; toast.classList.remove('is-hidden'); global.clearTimeout(showToast.timer); showToast.timer = global.setTimeout(function () { toast.classList.add('is-hidden'); }, 2200); }
    function showMessage(message, type) {
        if (!PAGE_CONFIG.mockMode && global.isloadpage && typeof global.isloadpage.openModal === 'function') {
            ...
            global.isloadpage.openModal({ hp: 'common', hf: 'common_msg', params: params, title: '提示', width: 400, height: 250 }); return;
        }
        showToast(message);
    }
    function downloadCurrent() {
        if (!currentDocument) { return; }
        dataService.download(currentDocument).then(function () { showMessage('文件下载已触发：' + currentDocument.fileName, 'success'); })
            .catch(function (error) { console.error(error); showMessage(error.message || '文件下载失败', 'error'); });
    }
    function returnToSource() {
        if (typeof global.closeModalDialog === 'function') { global.closeModalDialog(); return; }
        if (global.history.length > 1) { global.history.back(); return; }
        var params = commonParams(), query = new URLSearchParams({ act: 'hf', hp: params.hp, hf: queryParam('sourcePage') || 'syssjcj_cjwd_list', dbnm: params.dbnm });
        if (params.sessionId && !/^(none|null|undefined)$/i.test(String(params.sessionId))) { query.set('sessionId', params.sessionId); }
    ...
    function loadPage() {
        ...
        }).catch(function (error) { console.error('文件详情加载失败：', error); showError('文件信息加载失败，请返回列表后重试'); });
    }
    function bindEvents() {
        el('errorBackButton').addEventListener('click', returnToSource);
        el('downloadButton').addEventListener('click', downloadCurrent); el('unsupportedDownload').addEventListener('click', downloadCurrent);
        el('pdfPrevious').addEventListener('click', function () { if (pdfPage > 1) { pdfPage -= 1; renderPdfPage(); } });
        el('pdfNext').addEventListener('click', function () { if (pdfPage < 2) { pdfPage += 1; renderPdfPage(); } });
        el('imageZoomOut').addEventListener('click', function () { imageScale = Math.max(.6, imageScale - .2); updateImageScale(); });
        el('imageZoomIn').addEventListener('click', function () { imageScale = Math.min(1.8, imageScale + .2); updateImageScale(); });
        el('imageReset').addEventListener('click', function () { imageScale = 1; updateImageScale(); });
    }
    function initialize() { if (typeof global.initGlobalParams === 'function') { global.initGlobalParams(); } bindEvents(); loadPage(); }
