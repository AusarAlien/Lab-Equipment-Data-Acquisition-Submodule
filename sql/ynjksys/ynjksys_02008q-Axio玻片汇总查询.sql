...
begin
  id := 'ynjksys_02008q'; name := 'Axio玻片汇总查询'; direct := '0';
...
select slide_no 玻片号,
       max(group_name) 组别,
       max(case when item_name='细胞数' then result_value end) 细胞数,
       max(case when item_name='畸变细胞数' then result_value end) 畸变细胞数,
       max(case when item_name='畸变总数' then result_value end) 畸变总数,
       max(case when item_name='畸变率' then result_value end) 畸变率,
       max(case when item_name='断裂合计' then result_value end) 断裂,
       max(case when item_name='单体互换合计' then result_value end) 单体互换,
       max(case when item_name='裂隙合计' then result_value end) 裂隙,
       max(case when item_name='微小体合计' then result_value end) 微小体,
       max(case when item_name='有着丝点环合计' then result_value end) 有着丝点环,
       max(case when item_name='无着丝点环合计' then result_value end) 无着丝点环,
       max(case when item_name='双微小体合计' then result_value end) 双微小体,
       max(case when item_name='非特定性型变化合计' then result_value end) 非特定性型变化
  from axio_data
 group by slide_no
 order by case when regexp_like(slide_no, '^\d+$') then to_number(slide_no) else 999999 end, slide_no~';
