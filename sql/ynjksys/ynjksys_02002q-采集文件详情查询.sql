...
select d.fdiseq 文件序号, d.ffilenm 文件名称,
       ...
       upper(trim(p.finstno)) 仪器编号,
       case upper(trim(p.finstno)) when 'AGILENT-1200' then '安捷伦1200液相色谱仪'
         ...
         when 'SYNERGYH1' then 'BioTek Synergy H1多功能酶标仪' else '--' end 仪器设备,
       case upper(trim(p.finstno)) when 'AGILENT-1200' then 'Agilent1200'
         when 'BRUKER-MICROFLEX' then 'BrukerMicroflex'
         when 'AXIOIMAGERZ2' then 'AxioImagerZ2'
         when 'ICAP-TQ' then 'ICAP_TQ'
         when 'SYNERGYH1' then 'ExcelSYNERGYH1' else '--' end 处理接口,
