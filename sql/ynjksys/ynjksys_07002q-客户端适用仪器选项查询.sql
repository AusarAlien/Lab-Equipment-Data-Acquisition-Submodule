...
begin
  id := 'ynjksys_07002q'; name := '采集客户端适用仪器选项查询'; direct := '0';
  ...
  bsql := q'~select distinct m.FINSTNO 仪器编号,m.FINSTNM 仪器设备
  from HTLIS.LIS_CLIENT_PACKAGE_INST m
  join HTLIS.LIS_CLIENT_PACKAGE p on p.FPACKAGEID=m.FPACKAGEID and p.FHIINO=m.FHIINO
 where m.FHIINO=nvl(?,m.FHIINO)
   and p.FSTATUS='ENABLED'
 order by m.FINSTNM~';
  bsql_pv := 'hiino_sql_equal;';
  bsql_pt := 'N;';
