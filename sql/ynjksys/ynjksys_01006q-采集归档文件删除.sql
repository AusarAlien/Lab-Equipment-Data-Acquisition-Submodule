...
begin
  id := 'ynjksys_01006q'; name := '采集归档文件删除'; direct := '0';
  ...
  v_updated_rows number := 0 -FH-
  v_result_json varchar2(4000) -FH-
begin
  delete from q1 -FH-
  if nvl(trim(v_ifadmin), '0') <> '1' then
    raise_application_error(-20001, '当前账号无归档文件删除权限') -FH-
  end if -FH-
  ...
  if v_file_count <> 1 then
    raise_application_error(-20003, '未找到当前机构可删除的采集文件') -FH-
  end if -FH-

  delete from htlis.lis_instdata_new
   where fdiseq = v_fdiseq
     and fhiino = v_hiino -FH-
  v_deleted_rows := sql%rowcount -FH-

  update hii.ib_tbs_detailedinf
     set finvalidflag = '1',
         finvaliddt = sysdate,
         finvalidop = v_empid
   where fdiseq = v_fdiseq
     and fhiino = v_hiino
     and nvl(trim(finvalidflag), '0') = '0' -FH-
  v_updated_rows := sql%rowcount -FH-
  if v_updated_rows <> 1 then
    raise_application_error(-20004, '采集文件状态更新失败') -FH-
  end if -FH-

  commit -FH-
  v_result_json := '{"success":true,"message":"文件删除成功","deletedParseRows":'
                   || v_deleted_rows || '}' -FH-
...
select c1 处理结果 from q1~';
  bsql_pv := 'bodyjson_sql_equal,hiino_sql_equal,empId_sql_equal,ifAdmin_sql_equal;';
  bsql_pt := 'V,N,V,V;';
